#!/bin/bash
# Requires bc, gnuplot
# Usage: pingplot -a <address> -i <interval> -d (delete temporary files on exit) -x (Don't plot on exit)
...
if [ "$ping_address" = "" ]; then
	echo $(color lightred)"Invalid Input. Exiting."$(color)
	exit
fi
...
echo "Logging ping replies to"$(color cyan)" pinglog_avg"$(color)" and average ping to"$(color cyan) "ping_table "$(color)"in "$(color cyan)$(pwd)$(color)
echo "(p) to plot vs packets / (t) to plot vs time / (q) to exit"
echo "# Ping results for" $ping_address >> ping_table
echo -e "# Valid-Packets"'\t'"Average-Ping"'\t'"Current-Ping"'\t'"Dropped-Packets" >> ping_table
...
function finish {
	echo
	echo $(color lightgreen)"Done."$(color)
		if [[ ! $no_plot = 1 ]]; then
			plot_graph_packets
			plot_graph_time
		fi
...
do
	...
	if [ ! $final_line_length = 0 ]; then
		echo -e '\t\t\t\t\t'$(color lightgreen)"FOUND"$(color)
		echo
		break
	fi
done
